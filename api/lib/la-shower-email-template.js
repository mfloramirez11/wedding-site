/**
 * LA Baby Shower RSVP confirmation email
 * Event: Saturday, April 25, 2026 · 1:00 PM · 314 E 106th St, Los Angeles, CA
 */
export function generateLAShowerConfirmationEmail({ name, attending, guestCount, guests }) {
  let guestListHtml = '';
              ...
              <table role="presentation" style="width: 100%; border-collapse: collapse; background: rgba(224, 122, 170, 0.05); border-radius: 10px; border: 1px solid rgba(224, 122, 170, 0.2);">
                <tr>
                  <td style="padding: 28px; text-align: center;">
                    ...
                    <p style="font-family: Georgia, serif; font-style: italic; font-size: 20px; color: #6d2b47; margin: 0 0 5px;">Saturday, April 25th, 2026</p>
                    <p style="font-family: Georgia, serif; font-style: italic; font-size: 15px; color: #b05678; margin: 0 0 22px;">1:00 in the afternoon</p>
                    <p style="font-family: Georgia, serif; font-size: 15px; color: #8a3a58; margin: 0; line-height: 1.7;">314 E 106th St<br>Los Angeles, CA</p>
                  </td>
                </tr>
              </table>
            </td>
...
export function generateLAShowerConfirmationText({ name, attending, guestCount, guests }) {
  let guestList = '';
  if (attending === 'yes' && guests && guests.length > 0) {
    guestList = '\n\nYour Party:\n' + guests.map(guest => {
      let info = `  • ${guest.name}`;
